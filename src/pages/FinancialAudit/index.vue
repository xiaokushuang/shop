    ...
    <Pabody v-show="!isDetails" style="margin-top:20px">
      <PaTable>
        ...
        <tr v-for="(item, index) in list" :key="index">
          ...
          <td>{{ item.fo_review_status | AuditStatusOption }}</td>
          <td>
            <el-button
                    v-if="havePermission(120,'查看')"
                    style="width:97px;"
            ...
            <el-button
                    v-if="item.fo_review_status == null&&havePermission(121,'商户审核')"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      gotodetails(id,btn) {
        this.$router.push({
          path: `/FinancialAudit/details/${id}`,
          query: {
            flag: btn
          }
        });
